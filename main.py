...
def main(
    ...
    if is_training:
        ...
        val_set = Subset(train_set_clean, val_indices) # <- Hier ist der Zauber! Keine Augmentations!

        logger.info("Load data loaders...")
        train_loader = load_dataloaders(real_train_set, config["batch_size"], config["num_workers"])
        val_loader = load_dataloaders(val_set, config["batch_size"], config["num_workers"])
        ...
        test_loader = load_dataloaders(test_set, config["batch_size"], config["num_workers"])
        
        test_loss, test_acc = evaluate(model, test_loader, device)
        logger.info(f"Final test scores --> Loss: {test_loss:.4f} | Accuracy: {test_acc*100:.2f}%")